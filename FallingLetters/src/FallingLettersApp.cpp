...
class FallingLettersApp : public App {
public:
	void setup() override;
	...
	void add_box(const vec2& pos);

	b2World* mWorld;
	vector<b2Body*>		mBoxes;
};

void FallingLettersApp::setup()
{
...
void FallingLettersApp::add_box(const vec2& pos)
{
	b2BodyDef bodyDef;
...
void FallingLettersApp::mouseDown(MouseEvent event)
{
	add_box(event.getPos());
}

void FallingLettersApp::update()
{
	for (int i = 0; i < 10; ++i)
		mWorld->Step(1 / 30.0f, 10, 10);
}

void FallingLettersApp::draw()
{
	gl::clear();
...
CINDER_APP(FallingLettersApp, RendererGl)
